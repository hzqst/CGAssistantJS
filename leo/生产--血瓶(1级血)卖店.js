...
require(process.env.CGA_DIR_PATH_UTF8+'/leo').then(async (cga) => {
	...
	leo.log('全自动1级血瓶采集制作贩卖商店脚本，启动~');

	var doctorName = '假医生';
	var itemName = '生命力回复药（100）';
	var skillLevel = 1;
  ...
  var countNo = 1;
	...
	var main = async () => {
		...
		await leo.checkHealth(doctorName)
			//检查库存	
		if(cga.getItemCount('生命力回复药（100）') >= 3){
			await leo.goto(n=>n.castle.x)
			await leo.autoWalk([31,77])
			await leo.sell(30, 77, (item)=>{return item.name == '生命力回复药（100）' && item.count == 3})
		    await leo.logBack()}
		//1.苹果薄荷
		if(cga.getItemCount('苹果薄荷')<400){		
	...
	//制造血瓶
		if(cga.getItemCount('苹果薄荷')>= 400&& cga.getItemCount('柠檬草')>=40){
...
//			await leo.dropItem('生命力回复药（100）',3) // 扔掉不成组的，不然卡交易
//			await leo.delay(500)
			await leo.autoWalk([31,77])
			await leo.sell(30, 77, (item)=>{return item.name == '生命力回复药（100）' && item.count == 3})
