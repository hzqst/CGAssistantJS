    ...
    try{
        await leo.loop(async ()=>{
            ...
            if(isTeamLeader) {
                ...
                if (cga.GetMapName()=='黑龙沼泽') {
                    await leo.moveAround()
                    await leo.autoWalk([11,17,'*'])
                    //await leo.moveAround()
                ...
                if (!['艾尔莎岛','里谢里雅堡','法兰城','圣骑士营地','肯吉罗岛','黑龙沼泽1区','黑龙沼泽'].includes(cga.GetMapName())) {
                    await leo.logBack()
                }
            }else {
                await leo.loop(async ()=>{
                    ...
                    if (mapInfo.name.includes('黑龙沼泽')){
                        await leo.encounterTeammate(protect, '黑龙沼泽') //队员遇敌
                    }
                    await leo.delay(3000)
                })
            }
