require(process.env.CGA_DIR_PATH_UTF8+'/leo').then(async (cga) => {
    ...
    if(teammates == null){
        await leo.log('红叶の森林杀鸡脚本，未找到队伍，请确认配置是否正确')
        return leo.exit(1000*60*60*2);
    }else{
        await leo.log('红叶の森林杀鸡脚本，推荐01~10级使用，启动~');
        ...
        await leo.loop(async ()=>{
            ...
            if(isTeamLeader) {
                ...
                if (cga.GetMapName()=='盖雷布伦森林') {
                    ...
                    await leo.autoWalk([199,211,'艾夏岛'])
                }
                if (!['艾尔莎岛','艾夏岛','盖雷布伦森林'].includes(cga.GetMapName())) {
                    await leo.logBack()
                }
            }else {
                await leo.loop(async ()=>{
                    ...
                    if (mapInfo.name == '盖雷布伦森林'){
                        await leo.encounterTeammate(protect, '盖雷布伦森林') //队员遇敌
                    }
                    await leo.delay(3000)
                })
            }
