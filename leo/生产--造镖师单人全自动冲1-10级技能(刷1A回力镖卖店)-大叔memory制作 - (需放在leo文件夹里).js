...
require(process.env.CGA_DIR_PATH_UTF8+'/leo').then(async (cga) => {
	...
	leo.log('大叔memory尝试自采自造1级小刀制作脚本，需要使用leo的模块启动');
	var doctorName = '大叔-医治苍生';
	var countmany = 1; //制作次数
	var skillLevel = 1; //造几级， 目前只写了造1级的
	var productName="木制大型回力镖" ;
	var skill = cga.findPlayerSkill('造回力镖');
	if(!skill){
...
/*
		///////////////////////////////// 3.买布模块开始	
		leo.log('买布模块开始');	
		while(cga.getItemCount('麻布')<10 && cga.getItemCount('木制大型回力镖') < 1){
...
*/
		...
		while(cga.getItemCount('铜条')>=4 
			...
			await leo.log('材料已集齐，开始去制造一级1A 回力镖')
			await leo.logBack()
			await leo.checkHealth(doctorName)
			await leo.loop(()=>{
			...
			await leo.log('造回力镖完成第' + (countmany++) + '轮')
		}
