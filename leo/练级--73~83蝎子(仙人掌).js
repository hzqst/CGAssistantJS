require(process.env.CGA_DIR_PATH_UTF8+'/leo').then(async (cga) => {
    ...
    const fightType = 1;//0-蝎子，1-仙人掌
    const meetingPoint = 4; //集合点1~8
    
    leo.baseInfoPrint(); //显示基础信息
    let teammates = leo.findMyTeam(teams);
    if(teammates == null){
        await leo.log('红叶の蝎子(仙人掌)脚本，未找到队伍，请确认配置是否正确')
        return leo.exit(1000*60*60*2);
    }else{
        await leo.log('红叶の蝎子(仙人掌)脚本，推荐73~83级使用，启动~');
    ...
    try{
        await leo.loop(async ()=>{
            ...
            if(isTeamLeader) {
                ...
                if (cga.GetMapName()=='肯吉罗岛' && leo.getMapInfo().x > 300) {
                    await leo.autoWalkList([
                        [384, 245,'蜥蜴洞穴'],[12, 2,'肯吉罗岛'],[231,434,'矮人城镇']
                    ])
                }
                if (cga.GetMapName()=='矮人城镇') {
                    await leo.autoWalk([121, 110])
                    if(sellStone){
                        await leo.walkList([
                            [121, 109],[121, 110],[121, 109],[121, 110]
                        ])
                        await leo.sell(122, 110)
                        await leo.delay(5000)
                    }
                    await leo.autoWalk([163, 95])
                    await leo.supplyDir(0)
                    await leo.delay(2000)
                    await leo.statistics(leo.beginTime, leo.oldXp) //打印统计信息
                    await leo.autoWalk([110,191,'肯吉罗岛'])
                }
                if (cga.GetMapName() == '肯吉罗岛' && leo.getMapInfo().x < 300) {
                    let fightPos = [231,438];//蝎子
                    if(fightType==1){
                        fightPos = [231,435];//仙人掌
                    }
                    await leo.autoWalk([231,436])
                    await leo.autoWalk(fightPos)
                    console.log(leo.logTime() + '开始战斗');
                    await leo.encounterTeamLeader(protect) //队长遇敌
                    console.log(leo.logTime() + "触发回补");
                    await leo.delay(3000)
                    await leo.autoWalk([231,434,'矮人城镇'])
                    await leo.delay(1000)
                }
                if (!['艾尔莎岛','里谢里雅堡','法兰城','圣骑士营地','肯吉罗岛','矮人城镇'].includes(cga.GetMapName())) {
                    await leo.logBack()
                }
            }else {
                await leo.loop(async ()=>{
                    ...
                    if (mapInfo.name == '肯吉罗岛'){
                        await leo.encounterTeammate(protect, '肯吉罗岛') //队员遇敌
                    }
                    await leo.delay(3000)
                })
            }
