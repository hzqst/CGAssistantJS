...
require(process.env.CGA_DIR_PATH+'/leo').then(async (cga) => {
	...
	var productName="小圆盾" ;
	var skill = cga.findPlayerSkill('造盾');
	if(!skill){
		console.error('提示：没有造盾技能！');
		return;
	}
	if(skill.lv<skillLevel){
		console.error('提示：造盾技能等级不足，至少要'+skillLevel+'级技能！');
		return;
	}
	...
	var main = async () => {
		...
		///////////////////////////////// 2.买布模块开始	
		leo.log('买布模块开始');	
		while(cga.getItemCount('麻布')<10 && cga.getItemCount('小圆盾') < 1){
	...
	//		await leo.goto(n=>n.teleport.yer)
		...
		while(cga.getItemCount('铜条')>=4 
			...
			await leo.log('材料已集齐，开始去制造一级1A 小圆盾')
			await leo.logBack()
			await leo.checkHealth(doctorName)
			await leo.loop(()=>{
				...
				if (cga.getItemCount('铜条') < 4 || cga.getItemCount('麻布') < 1) {
					return leo.reject();
				}
			...
			await leo.delay(500)
			await leo.turnDir(0)
			await leo.log('本轮采集+制造1级盾完成')
			await leo.log('造盾完成第' + (countmany++) + '轮')
		}
