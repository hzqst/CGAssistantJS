require(process.env.CGA_DIR_PATH_UTF8+'/leo').then(async (cga) => {
    ...
    const huizhang = async () => {
        if(leo.has('矮人徽章') || leo.has('巴萨的破酒瓶')){
            return leo.next();
        }else{
            ...
            await leo.autoWalkList([[0,23,'圣骑士营地'],[97,85]])
        }
    }
    ...
    try{
        await leo.loop(async ()=>{
            ...
            if(isTeamLeader) {
                ...
                if (cga.GetMapName()=='矮人城镇' && leo.getMapInfo().x > 93) {
                    await leo.autoWalk([163, 95])
                    await leo.supplyDir(0)
                    await leo.delay(2000)
                    await leo.statistics(leo.beginTime, leo.oldXp) //打印统计信息
                    await leo.autoWalk([97,124])
                    await leo.loop(async ()=>{
                        if(leo.getMapInfo().x < 93) {
                            return leo.reject();
                        }
                ...
                await leo.loop(async ()=>{
                    ...
                    if (mapInfo.name == '矮人城镇' && mapInfo.x > 150) {
                        await leo.statistics(leo.beginTime, leo.oldXp) //打印统计信息
                        await leo.delay(20000)
                    }
                    if (mapInfo.name == '矮人城镇' && mapInfo.x < 93) {
                        await leo.encounterTeammate(protect,()=>{
                            if(leo.getMapInfo().x > 93) return true;
                        }) //队员遇敌
                    }
                    await leo.delay(3000)
                })
