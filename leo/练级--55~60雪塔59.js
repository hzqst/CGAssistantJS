require(process.env.CGA_DIR_PATH_UTF8+'/leo').then(async (cga) => {
    ...
    const meetingPoint = 3; //集合点1~3
    
    leo.baseInfoPrint(); //显示基础信息
    let teammates = leo.findMyTeam(teams);
    if(teammates == null){
        await leo.log('红叶の雪塔59脚本，未找到队伍，请确认配置是否正确')
        return leo.exit(1000*60*60*2);
    }else{
        await leo.log('红叶の雪塔59脚本，推荐55~60级使用，启动~');
    ...
    try{
        await leo.loop(async ()=>{
            ...
            if(isTeamLeader) {
                ...
                if (!['艾尔莎岛','国民会馆','雪拉威森塔５９层'].includes(cga.GetMapName())) {
                    await leo.logBack()
                }
            }else {
                await leo.loop(async ()=>{
                    ...
                    if (mapInfo.name == '雪拉威森塔５９层'){
                        await leo.encounterTeammate(protect, '雪拉威森塔５９层') //队员遇敌
                    }
                    await leo.delay(3000)
                })
            }
