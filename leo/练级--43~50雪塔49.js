    ...
    try{
        await leo.loop(async ()=>{
            ...
            if(isTeamLeader) {
                ...
                if (!['艾尔莎岛','国民会馆','雪拉威森塔４９层'].includes(cga.GetMapName())) {
                    await leo.logBack()
                }
            }else {
                await leo.loop(async ()=>{
                    ...
                    if (mapInfo.name == '雪拉威森塔４９层'){
                        await leo.encounterTeammate(protect, '雪拉威森塔４９层') //队员遇敌
                    }
                    await leo.delay(3000)
                })
            }
