require(process.env.CGA_DIR_PATH+'/leo').then(async (cga) => {
    ...
    if(teammates == null){
        await leo.log('红叶の雪塔49脚本，未找到队伍，请确认配置是否正确')
        return leo.delay(1000*60*60*2);
    }else{
        await leo.log('红叶の雪塔49脚本，推荐43~50级使用，启动~');
    ...
    leo.todo().then(() => {
        ...
        return leo.loop(
            () => leo.waitAfterBattle()
            .then(() => leo.checkHealth(prepareOptions.doctorName,false))
            ...
            .then(()=>{
                if(!isTeamLeader){
                    var pet = cga.GetPetsInfo().find(p => p.index == 0);
                    //console.log(pet);
                    if(pet && pet.state!=2){
            ...
            .then(() => {
                ...
                if (isTeamLeader) {
                    ...
                    if (mapInfo.name.indexOf('雪拉威森塔４９层')!=-1){
                        return leo.encounterTeammate(protect, '雪拉威森塔４９层'); //队员遇敌
                    }
                }
                //console.log('延时3秒');
