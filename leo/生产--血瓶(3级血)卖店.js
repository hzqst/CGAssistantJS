...
require(process.env.CGA_DIR_PATH+'/leo').then(async (cga) => {
  ...
  var countNo = 1;
	...
	//检查库存	
		if(cga.getItemCount('生命力回复药（200）') >= 3){
			await leo.goto(n=>n.castle.x)
			await leo.autoWalk([31,77])
			await leo.sell(30, 77, (item)=>{return item.name == '生命力回复药（200）' && item.count == 3})
		    await leo.logBack()}
		//1.苹果薄荷
		if(cga.getItemCount('苹果薄荷')<240){		
		...
		//2.柠檬草
		if(cga.getItemCount('柠檬草')< 240){
			var mapInfo = cga.getMapInfo();
			if(cga.getItemCount('苹果薄荷') >= 240&& mapInfo.name == '艾尔莎岛'){
				await leo.goto(n=>n.castle.x)
				await leo.autoWalk([34,88])
				await leo.supplyDir(0)
				await leo.goto(n=>n.falan.wout)
			...
			//3.蝴蝶花
		if(cga.getItemCount('蝴蝶花')< 120){
			var mapInfo = cga.getMapInfo();
			if(cga.getItemCount('蝴蝶花') >= 240&& mapInfo.name == '艾尔莎岛'){
				await leo.goto(n=>n.castle.x)
				await leo.autoWalk([34,88])
				await leo.supplyDir(0)
				await leo.goto(n=>n.falan.wout)
			...
			await leo.autoWalk([503, 118])           
            await leo.log('到达位置，开始采花蝴蝶花')
			await leo.loop(()=>{
				if(cga.GetPlayerInfo().mp < 1){
					return leo.log('魔力不足')
					.then(()=>leo.reject());
				}
				var count = cga.getItemCount('蝴蝶花');
				if (count >= 120) {
					return leo.reject();
				}
				cga.StartWork(famuSkill.index, 0);
			...
			await leo.dropItem('蝴蝶花',40)
			
		}	
	
	//制造血瓶
		if(cga.getItemCount('苹果薄荷')>= 240&& cga.getItemCount('蝴蝶花')>=120){
...
//			await leo.dropItem('生命力回复药（200）',3) // 扔掉不成组的，不然卡交易
//			await leo.delay(500)
			await leo.autoWalk([31,77])
			await leo.sell(30, 77, (item)=>{return item.name == '生命力回复药（200）' && item.count == 3})
