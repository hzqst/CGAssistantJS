  ...
  var countNo = 1;
	...
	var main = async () => {
		...
		if(cga.getItemCount('胡椒')>=160
			...
			await leo.pile(true)
			await leo.delay(500)
//			await leo.dropItem('烧鸡',3) // 扔掉不成组的，不然卡交易
//			await leo.delay(500)
			await leo.autoWalk([31,77])
			await leo.sell(30, 77, (item)=>{return item.name == '烧鸡' && item.count == 3})
			await leo.log('本轮采集+烧鸡制造贩卖完成')
			await leo.log('厨师单人全自动采集制作烧鸡贩卖商店，完成第' + (countNo++) + '次')
		}
	}
