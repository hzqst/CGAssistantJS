var cga = require('./cgaapi')(function(){
	console.log('开始执行模块：刷双百-鞋子，需要鹿皮脚本');
	...
	var docraft = ()=>{
		...
		if(skill.lv >= 3){
			...
			if(cga.getItemCount('麻布') < 5 && cga.getItemCount('木棉布') < 4 && cga.getItemCount('毛毡') < 4){
				cga.craft.buyFabricLv1Multi([
				{index:0, count:5},
				{index:1, count:4},
				{index:2, count:4},
				], (r)=>{
			...
			if(cga.getItemCount('木棉布') < 4){
				cga.craft.buyFabricLv1(1, 4, (r)=>{
					docraft();
				});
				return;
			}
			...
			craftItemName = '皮鞋';
			if(cga.getItemCount('麻布') < 4 && cga.getItemCount('木棉布') < 4){
				cga.craft.buyFabricLv1Multi([
				{index:0, count:4},
				{index:1, count:4},
				], (r)=>{
			...
			if(cga.getItemCount('麻布') < 4){
				cga.craft.buyFabricLv1(0, 4, (r)=>{
					docraft();
				});
				return;
			}
		...
		if(items.length > 20 - trade_count){
			cga.travel.falan.toFabricStore(()=>{
				...
				[7, 7]
					...
					cga.TurnTo(7, 5);
					var sellarray = cga.findItemArray('光之鞋');
					var sellarray2 = cga.findItemArray('皮鞋');
					var sellarray3 = cga.findItemArray('运动鞋');
					cga.sellArray(sellarray.concat(sellarray2, sellarray3), ()=>{
						setTimeout(doWork, 1000);
					});
				})
			});	
